...
const quickActions = [
  ...
  { label: "Generate Report", icon: FileBarChart, action: "report" },
  { label: "Create Document", icon: FilePlus2, action: "document" },
  { label: "Analyze Data", icon: ChartBar, action: "analyze" },
];
...
export function ChatComposer() {
  ...
  const handleQuickAction = (action: string) => {
    ...
    addNotification({
      title: `${actionLabels[action]} requested`,
      message: `The AI assistant is preparing to ${actionLabels[action].toLowerCase()}.`,
      time: "just now",
    });

    sendMessage(`Please ${actionLabels[action].toLowerCase()} for my business.`);
  };
  ...
  return (
    <div className="border-t border-border bg-background p-4">
      ...
      <div className="mt-2.5 flex flex-wrap gap-2">
        ...
        <Button variant="outline" size="sm" className="text-xs" onClick={() => handleQuickAction("analyze")}
        >
